...
### Corrigido (revisão de consistência)

- **`PyPlayground` vazava estado do Python entre execuções**: como o
  interpretador Pyodide é compartilhado (um único carregamento, reusado
  por todos os `PyPlayground` da página), variáveis definidas em um
  clique de "Rodar" continuavam vivas no clique seguinte — e até entre
  playgrounds diferentes na mesma página (a Aula 10/M1 tem dois). Corrigido
  passando um namespace Python novo (`pyodide.toPy({})`) para cada
  chamada de `runPythonAsync`, destruído ao final da execução. O botão
  "Reiniciar" já limpava o estado do React; agora cada "Rodar" também
  garante um estado limpo do lado do Python.
