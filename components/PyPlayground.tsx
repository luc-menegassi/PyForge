...
export function PyPlayground({
  ...
  async function rodar() {
    ...
    try {
      ...
      // Cada clique em "Rodar" ganha um namespace Python novo e vazio.
      // Sem isso, o interpretador (que é compartilhado — ver getPyodide
      // acima) manteria variáveis de execuções anteriores vivas entre um
      // clique e outro, e até entre PyPlaygrounds diferentes na mesma
      // página (a Aula 10/M1, por exemplo, tem dois).
      const namespace = pyodide.toPy({});

      let codigoFinal = codigo;
      if (usaInput) {
        const fila = entradas.split("\n");
        namespace.set("_pyforge_input_queue", pyodide.toPy(fila));
        codigoFinal = PREAMBULO_INPUT + "\n" + codigo;
      }

      try {
        await pyodide.runPythonAsync(codigoFinal, { globals: namespace });
      } finally {
        namespace.destroy();
      }
